...
//                   cy.selectFromDropdown("#mainNewBusiness\\/mainDetails\\/mainDetailsGPFCust\\.retirementAge","62");
//                   cy.get("#mainNewBusiness\\/mainDetails\\/mainDetailsGPFCust\\.button_7",{timeout:100000}).click();
...
//                   cy.get("#mainNewBusiness\\/contributions\\/premiumForLayer").get("#vaadin-text-field-input-66 > slot:nth-child(2) > input").type("1500");
...
//                       cy.wrap(dropdown).get("#content").find("[part='content']").contains("Fund").click({force:true});
//                       cy.get("#uiFunds\\/uiFundsGpfCust\\.profileCategory1Fund",{timeout:100000}).click();
//                       cy.selectFromDropdown("#uiFunds\\/uiFundsGpfCust\\.profileCategory1Fund","Equity");
//                       cy.get("#uiFunds\\/uiFundsGpfCust\\.investmentName1",{timeout:100000}).click();
//                       cy.selectFromDropdown("#uiFunds\\/uiFundsGpfCust\\.investmentName1","Alfred Berg Norge");
...
//                 cy.insertText("#NewPaymentBatch\\/amount",1500);
//                   cy.get("#paymentTableLabel_lightbox_done",{timeout:100000}).click();
                        
//                   cy.get("@policyNum").then(wags => {
//                     cy.get("#vaadin-text-field-input-124 > slot:nth-child(2) > input").type("RT" + wags, {force:true});
...
//           cy.get("#Batch\\/batch\\.deselectAll",{timeout:100000},{timeout:100000}).click();
//           cy.get("#Batch\\/table_26 > vaadin-grid-cell-content:nth-child(3)",{timeout:100000}).click();
...
//           cy.get("#UnitDealingTabs\\/multiview > div.spml-multiview-label-and-tabs > vaadin-tabs > vaadin-tab:nth-child(2)",{timeout:100000}).click();


//           cy.get("#alis-content-spml > div > vaadin-split-layout > div:nth-child(1) > div.views-placeholder > div > div").find("#UnitDealingQuery\\/searchButton").should("be.visible").click({force:true,multiple:true});
//           cy.get("@orderReference").then((order) => {
//             cy.get("#UnitDealingQuery\\/colOrderReference").last().scrollIntoView().dblclick({force:true});
//           });



//           //cy.get("#UnitDealingQuery\\/searchResultstable",{timeout:100000}).find("vaadin-grid-cell-content").contains("@orderReference").click({force:true});

//           cy.get("#UnitDealingQuery\\/detailsTable > vaadin-grid-cell-content:nth-child(9)",{timeout:100000}).click({force:true});
//           cy.get("#instructionDetailsView_openLightbox").click({force:true});
...
//           cy.get("#shell\\/menu\\/batch",{timeout:100000}).click();
//           cy.selectFromDropdown("#Batch\\/batch\\.batchName","Unit Dealing (Testing)");
//           cy.get("#Batch\\/batch\\.deselectAll").click();
