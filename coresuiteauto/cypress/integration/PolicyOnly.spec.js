...
import testData from "../fixtures/FK_over_67.json"
      describe('Creates New policy', () => {
        ...
        beforeEach(function () {
          ...
          cy.fixture("FK_over_67").then(test => {
            ...
            cy.selectFromDropdownTest("#mainNewBusiness\\/mainDetails\\/mainDetailsGPFCust\\.retirementAge",this.testdata.Main_details.find(compare).IntendedRetirementAge).then(()=>{
              ...
              if(clientExist){
                ...
                cy.wait(2000);
                    ...
                    cy.wait(2000);
                      cy.get("#PaymentArrangementGrid\\/day").click().then(()=>{
                        cy.get("body").find("vaadin-combo-box-overlay").not("hidden").eq(0).find("#selector").find("#content").contains(this.testdata.Person_details.find(compare).PaymentArrangementDay).click({multiple:true});
    
                      })
               cy.wait(2000);
               if(this.testdata.Person_details.find(compare).PaymentArrangementFrequency==="Yearly"){
               cy.get("  #PaymentArrangementGrid\\/month").click().then(()=>{
                cy.wait(2000);
                cy.get("body").find("vaadin-combo-box-overlay").not("hidden").eq(0).find("#selector").find("vaadin-combo-box-item").find("#content").contains("June").click({multiple:true});
              })
            }
...
const exists = (selector) => {
  return new Promise((resolve) => {
    cy.get("body").then(body => {
      const len = body.find(selector, {timeout: 10000}).length
